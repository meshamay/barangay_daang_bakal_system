...
async function submitComplaintForm() {
    ...
    // Validate complaint type (hidden input - browser can't show native tooltip for it)
    if (complaintTypeInput && !complaintTypeInput.value) {
        const btn = document.getElementById("complaintTypeButton");
        if (btn) {
            btn.style.borderColor = "#ef4444";
            btn.focus();
            // Create a temporary visible input to anchor the native tooltip
            const tmp = document.createElement("input");
            tmp.type = "text";
            tmp.required = true;
            tmp.style.cssText =
                "position:absolute;bottom:0;left:50%;transform:translateX(-50%);width:1px;height:1px;opacity:0.01;pointer-events:none;";
            btn.parentElement.style.position = "relative";
            btn.parentElement.appendChild(tmp);
            tmp.setCustomValidity("Please select a complaint type.");
            tmp.reportValidity();
            setTimeout(() => {
                if (tmp.parentNode) tmp.parentNode.removeChild(tmp);
            }, 3000);
            // Reset border when user selects a type
            const observer = new MutationObserver(() => {
                if (complaintTypeInput.value) {
                    btn.style.borderColor = "";
                    observer.disconnect();
                }
            });
            observer.observe(complaintTypeInput, {
                attributes: true,
                attributeFilter: ["value"],
            });
            complaintTypeInput.addEventListener(
                "change",
                () => {
                    btn.style.borderColor = "";
                },
    ...
    // Use browser's native required-field validation for all other fields
    ...
    try {
        ...
        if (response.status === 422) {
            ...
            form.reset();
            // Reset complaint type label
            const labelEl = document.getElementById("complaintTypeLabel");
            if (labelEl) {
                labelEl.textContent = "Select complaint type";
            ...
            if (errorsDiv) {
                errorsDiv.innerHTML =
                    ...
                    (data.details || data.message || "Please try again.");
                errorsDiv.classList.remove("hidden");
            }
        }
    } catch (error) {
